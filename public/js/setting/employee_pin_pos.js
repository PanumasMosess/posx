...
$formAddEmployeePinPos
    ...
    .on('click', '.btnAddEmployeePinPos', function (e) {
        ...
        // เช็คข้อมูล
        if ($formAddEmployeePinPos.find('input[name=username_employee_pos]').val() == '') {
            alert('กรุณาระบุชื่อ')
            return false;
        }
        else if ($formAddEmployeePinPos.find('input[name=new_password_employee_pos]').val() == '') {
            alert('กรุณากรอกรหัสผ่าน')
            return false;
        }
        else if ($formAddEmployeePinPos.find('input[name=confirm_password_employee_pos]').val() == '') {
            alert('กรุณากรอกยืนยันรหัสผ่าน')
            return false;
        }
        else if ($formAddEmployeePinPos.find('input[name=new_password_employee_pos]').val() != $formAddEmployeePinPos.find('input[name=confirm_password_employee_pos]').val()) {
        ...
        // ผ่าน
            ...
            formData.append('content', $formAddEmployeePinPos.find('.ql-editor').html())
...
function loadTableEmployeePinPos() {
    ...
    $("#tableEmployeePinPos").DataTable({
            ...
            render: function (data, type, row, meta) {
                ...
                if (all === 'checked') {
                    return ('<label class="custom-control-All custom-checkbox custom-control-md mb-1" style="margin-right: 10px;"><input type="checkbox" class="custom-control-input checkbox_All" name="checkbox_All" id="checkbox_All" ' + all + '><span class="custom-control-label custom-control-label-md ms-1" style="vertical-align: text-bottom;">All</span></label>' +
                        ...
                        '<label class="custom-control custom-checkbox custom-control-md mb-1" style="margin-right: 10px;display: none;"><input type="checkbox" class="custom-control-input" name="checkbox_Void" id="checkbox_Void" ' + pin_pos_void + '><span class="custom-control-label custom-control-label-md ms-1" style="vertical-align: text-bottom;">Void</span></label>');
                }
                ...
                return ('<label class="custom-control-All custom-checkbox custom-control-md mb-1" style="margin-right: 10px;"><input type="checkbox" class="custom-control-input checkbox_All" name="checkbox_All" id="checkbox_All" ' + all + '><span class="custom-control-label custom-control-label-md ms-1" style="vertical-align: text-bottom;">All</span></label>' +
                    ...
                    '<label class="custom-control custom-checkbox custom-control-md mb-1" style="margin-right: 10px;"><input type="checkbox" class="custom-control-input" name="checkbox_Void" id="checkbox_Void" ' + pin_pos_void + '><span class="custom-control-label custom-control-label-md ms-1" style="vertical-align: text-bottom;">Void</span></label>')
            },
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, meta) {
                return (
                    '<div class="action_btns d-flex" style="justify-content: center;"><button type="button" class="btn btn-primary f_s_14 saveEditEmployeePinPos me-2" data-id="' + data["id"] + '"><i class="ti-save f_s_13 me-2"></i>Save</button><button type="button" class="btn btn-danger f_s_14 btnDeleteEmployeePinPos" data-id="' + data["id"] + '"><i class="fas fa-trash f_s_13 me-2"></i>Remove</button></div>'
                );
            },
        }
        ],
