$(document).ready(function () {
  ...
  addUserBtnButton.addEventListener("click", function () {
    ...
    var editUserElements = document.querySelectorAll(
      ".editUser.bg-warning-light"
    );
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          error: function () {
            Swal.fire("ลบไม่สำเร็จ", response.message, "error");
          },
        });
      }
    });
...
function updateListItemRoles(employeeId, newRoles) {
  console.log(
    "Updating roles for employeeId: " + employeeId + ", newRoles: " + newRoles
  );
  var listItems = document.querySelectorAll(
    '.A.list-group a[data-id="' + employeeId + '"]'
  );
  listItems.forEach(function (listItem) {
    var rolesSpan = listItem.querySelector("span.roles");
    console.log(rolesSpan);
    if (rolesSpan) {
      rolesSpan.textContent =
        " ( " + (newRoles == "1" ? "Admin" : "Custom User") + " )";
    }
  });
}
...
$("body").on("click", "#saveNewUser", function () {
  const userList = document.querySelectorAll(".list-group-item").length;
  let maxUsers = 0;

  if (accountType === "1") {
    maxUsers = 3; // จำกัดจำนวนผู้ใช้ได้ 3 คน
  } else if (accountType === "2") {
    maxUsers = 5; // จำกัดจำนวนผู้ใช้ได้ 5 คน
  } else if (accountType === "3") {
    maxUsers = 10; // จำกัดจำนวนผู้ใช้ได้ 10 คน
  }
  ...
  if (userList >= maxUsers) {
    alert(`คุณไม่สามารถเพิ่ม User Account ได้มากกว่า ${maxUsers} คน`);
  } else {
    $.ajax({
      type: "POST",
